...
const cleanupStream = () => {
    console.log('cleanupStream 호출됨');
    
    // AbortController 정리 (HTTP 요청 중단용)
    if (abortController && !abortController.signal.aborted) {
        console.log('AbortController.abort() 호출');
    ...
    try {
        ...
        if (titleMatch || contentMatch) {
            ...
            for (const line of lines) {
                ...
                if (trimmedLine.startsWith('**제목:**') || trimmedLine.startsWith('제목:')) {
                    title = trimmedLine.replace(/^\*?\*?제목:\*?\*?\s*/, '');
                } else if (trimmedLine.startsWith('**내용:**') || trimmedLine.startsWith('내용:')) {
                    inContent = true;
                    const contentStart = trimmedLine.replace(/^\*?\*?내용:\*?\*?\s*/, '');
...
const stopGeneration = async () => {
    console.log('stopGeneration 호출됨 - 사용자가 중단 버튼 클릭');
    ...
    if (currentSessionId.value) {
        try {
            console.log(`취소 API 호출 (세션: ${currentSessionId.value})`);
            ...
            if (result.success) {
                console.log('서버에서 스트림 취소 완료:', result.message);
                cancelMessage.value = '위키 생성이 성공적으로 취소되었습니다.';
            } else {
                console.warn('서버 취소 요청 실패:', result);
                cancelMessage.value = '취소 요청 중 문제가 발생했지만 로컬에서 중단되었습니다.';
            }
        } catch (error) {
            console.error('취소 API 호출 실패:', error);
...
<template>
    ...
    <Teleport to="body">
        <div v-if="openModal" class="fixed inset-0 bg-black/50 flex flex-col items-center justify-center z-50 p-4">
            <div class="bg-[var(--ui-bg)] rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                <!-- 헤더 -->
                ...
                <div class="p-6 space-y-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid gap-2">
                                <div class="bg-[var(--ui-bg-elevated)] rounded-lg p-3 text-sm">
                                    <span class="text-[var(--ui-text-secondary)]">• "Vue.js의 Composition API에 대한 상세한
                                        가이드를 작성해주세요"</span>
                                </div>
                                <div class="bg-[var(--ui-bg-elevated)] rounded-lg p-3 text-sm">
                                    <span class="text-[var(--ui-text-secondary)]">• "TypeScript 기초부터 고급 기능까지 단계별로 설명하는
                                        문서"</span>
                                </div>
                                <div class="bg-[var(--ui-bg-elevated)] rounded-lg p-3 text-sm">
                                    <span class="text-[var(--ui-text-secondary)]">• "React Hook 사용법과 커스텀 Hook 만들기
                                        튜토리얼"</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div v-if="isGenerating || streamOutput || isStreamCancelled" class="space-y-4">
                        ...
                        <div v-else-if="isGenerating || streamOutput" class="space-y-4">
                            ...
                            <!-- 스트림 출력 -->
                            <div class="bg-[var(--ui-bg-elevated)] rounded-lg border border-[var(--ui-border)] overflow-hidden">
                                <!-- 헤더 -->
                                <div class="flex items-center gap-2 px-4 py-3 border-b border-[var(--ui-border)] bg-[var(--ui-bg)]">
                                    ...
                                    <div v-if="isGenerating && !isStreamCancelled" class="ml-auto flex items-center gap-1">
                                        <div class="w-1.5 h-1.5 bg-[var(--ui-secondary)] rounded-full animate-pulse"></div>
                                        <div class="w-1.5 h-1.5 bg-[var(--ui-secondary)] rounded-full animate-pulse" style="animation-delay: 0.2s"></div>
                                        <div class="w-1.5 h-1.5 bg-[var(--ui-secondary)] rounded-full animate-pulse" style="animation-delay: 0.4s"></div>
                                    </div>
                                </div>
                                ...
                                <div ref="streamContainer" class="p-4 max-h-64 overflow-y-auto scroll-smooth">
                                    ...
                                    <div v-if="isGenerating && !streamOutput" class="space-y-3">
                                        <div class="flex items-start gap-3">
                                            <div class="w-2 h-2 bg-[var(--ui-secondary)] rounded-full mt-2 animate-pulse"></div>
                                            <div class="flex-1">
                                                <div class="text-sm text-[var(--ui-text-secondary)] italic">
                                                    AI가 생각하고 있습니다...
                                                </div>
                                                <div class="mt-1 space-y-1">
                                                    <div class="h-2 bg-[var(--ui-border)] rounded animate-pulse w-3/4"></div>
                                                    <div class="h-2 bg-[var(--ui-border)] rounded animate-pulse w-1/2"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div v-if="streamOutput" class="space-y-2">
                                        <div class="flex items-start gap-3">
                                            <div class="w-2 h-2 bg-[var(--ui-secondary)] rounded-full mt-2 flex-shrink-0"></div>
                                            <div class="flex-1 min-w-0">
                                                <div class="text-xs text-[var(--ui-text-secondary)] mb-1">AI 응답</div>
                                                <div class="relative">
                                                    <pre class="whitespace-pre-wrap text-sm text-[var(--ui-text-muted)] italic leading-relaxed break-words">{{ streamOutput }}</pre>
                                                    <!-- 타이핑 커서 (생성 중일 때만) -->
                                                    <span v-if="isGenerating && !isStreamCancelled" class="typing-cursor">|</span>
                                                </div>
                                                <div v-if="isGenerating && !isStreamCancelled" class="mt-2 flex items-center gap-1">
                                    ...
                                    <div v-if="!isGenerating && !streamOutput" class="flex items-center justify-center py-8">
                                        <div class="text-center text-[var(--ui-text-secondary)]">
                                            <Icon icon="ph:brain" width="32" height="32" class="mx-auto mb-2 opacity-50" />
                ...
                <!-- 푸터 -->
                <div
                    class="flex items-center justify-between p-6 border-t border-[var(--ui-border)] bg-[var(--ui-bg-elevated)] rounded-b-xl">
                    <!-- 중단된 상태일 때 -->
