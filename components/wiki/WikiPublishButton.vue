...
const checkTitleExists = async () => {
    ...
    try {
        const response = await $fetch('/api/wiki/check-title', {
            method: 'POST',
            body: {
...
<template>
    <div class="relative">
        <button v-if="!isPublished" ref="openButton" @click="handleOpenModal"
        ...
        <button v-else ref="openButton" @click="handleUnpublish"
            class="action-button bg-gradient-to-r from-[var(--ui-secondary)] to-[var(--ui-secondary-muted)] text-white hover:from-[var(--ui-secondary-muted)] hover:to-[var(--ui-secondary-elevated)]">
