...
export async function getWikiHistoriesByWikiId(wikiId: string): Promise<DbResult<WikiHistory[]>> {
    const db = getFirestore(app);
    const docRef = query(collection(db, WIKI_HISTORY_COLLECTION), where("wikiId", "==", wikiId));
    const docSnap = await getDocs(docRef);
    if (docSnap.size > 0) {
        const histories = docSnap.docs.map((doc) => WIKI_HISTORY_SCHEMA.safeParse({ id: doc.id, ...doc.data() }));
        return { success: true, data: histories.map((history) => history.data) as WikiHistory[] };
    } else {
        return { success: false, error: { message: "Wiki history not found" } };
    }
}

export async function getWikiHistory(id: string): Promise<DbResult<WikiHistory>> {
    const db = getFirestore(app);
...
export async function setWikiHistory(data: Omit<WikiHistory, 'id'>): Promise<DbResult<WikiHistory>> {
    const db = getFirestore(app);
    const historyId = v4();
...
export async function updateWikiHistory(documentId: string, data: Partial<Omit<WikiHistory, 'id'>>): Promise<DbResult<WikiHistory>> {
    const db = getFirestore(app);
...
export async function deleteWikiHistory(documentId: string): Promise<DbResult<WikiHistory>> {
    const db = getFirestore(app);
    const docRef = doc(db, WIKI_HISTORY_COLLECTION, documentId);
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
...
export async function deleteWikiHistoriesByWikiId(wikiId: string): Promise<DbResult<void>> {
    const db = getFirestore(app);
    const docRef = query(collection(db, WIKI_HISTORY_COLLECTION), where("wikiId", "==", wikiId));
